...
type UpdateMyselfParams struct {
	Email          string
	HashedPassword string
	ID             uuid.UUID
}

func (q *Queries) UpdateMyself(ctx context.Context, arg UpdateMyselfParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateMyself, arg.Email, arg.HashedPassword, arg.ID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Email,
...
const updateToChirpyRed = `-- name: UpdateToChirpyRed :execrows
UPDATE users
SET is_chirpy_red = TRUE, updated_at = NOW()
WHERE id = $1
`

func (q *Queries) UpdateToChirpyRed(ctx context.Context, id uuid.UUID) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateToChirpyRed, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
